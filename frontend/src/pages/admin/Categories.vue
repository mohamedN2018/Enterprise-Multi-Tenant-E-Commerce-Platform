...
const blank = () => ({ name: '', name_en: '', description: '', position: 0, is_active: true });
...
const openEdit = (c) => {
  editing.value = c;
  form.value = { name: c.name, name_en: c.name_en || '', description: c.description || '', position: c.position, is_active: c.is_active };
  showModal.value = true;
};
const save = async () => {
  if (!run()) return;
