...
const metrics = computed(() => {
  ...
  const d = dash.value;
  return [
    { label: 'Orders to process', value: d.orders.pending, icon: Clock, tone: 'bg-amber-50 text-amber-700' },
    { label: 'Low-stock items', value: d.catalog.low_stock, icon: AlertTriangle, tone: 'bg-secondary-50 text-secondary-700' },
    { label: 'Published products', value: d.catalog.published, icon: Package, tone: 'bg-primary-50 text-primary-700' },
    { label: 'Store rating', value: (d.reviews.average || 0).toFixed(1), icon: Star, tone: 'bg-sky-50 text-sky-700' }
  ];
});

const shortcuts = [
  { label: 'Orders', desc: 'Review incoming orders', icon: ShoppingBag, to: { name: 'admin-orders' } },
  { label: 'Products', desc: 'Browse the catalog', icon: Package, to: { name: 'admin-products' } },
  { label: 'Inventory', desc: 'Check stock levels', icon: Boxes, to: { name: 'admin-inventory' } },
  { label: 'Categories', desc: 'View categories', icon: Tags, to: { name: 'admin-categories' } }
];
...
<template>
  <div>
    <div v-if="loading" class="flex min-h-[50vh] items-center justify-center"><Spinner :size="30" label="Loading…" /></div>

    <template v-else>
      <PageHeader :title="`Hi, ${auth.displayName} 👋`" :subtitle="`${tenant.active?.name} · staff view`">
        <template #actions>
          <span class="chip border-0 bg-slate-100 text-slate-600">Employee · read-only</span>
        </template>
      </PageHeader>
      ...
      <!-- Shortcuts -->
      <h3 class="mb-3 mt-8 font-heading text-lg font-semibold">Quick access</h3>
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
