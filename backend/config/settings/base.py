...
MIDDLEWARE = [
    ...
    # Serves collected static files (admin / DRF / Swagger UI) without a separate
    # web server — works behind the frontend nginx reverse proxy on Dokploy.
...
# WhiteNoise compresses collected static at `collectstatic` time (gzip/brotli),
# without manifest hashing — robust even if a third-party asset references a
# missing map file.
_STATIC_STORAGE = "whitenoise.storage.CompressedStaticFilesStorage"
if USE_S3:
    STORAGES = {
