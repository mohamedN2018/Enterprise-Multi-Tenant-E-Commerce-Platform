"""Shared fixtures for fraud tests."""
...
def buyer_setup(store_client, make_user):
    """Published, stocked variant + a buyer client. Returns (client, buyer, variant)."""
    counter = {"n": 0}
...
def place_order(client, variant, qty=1) -> dict:
    client.post(
        reverse("v1:cart:item-add"),
        {"variant_id": str(variant.id), "quantity": qty},
        format="json",
    )
    return client.post(reverse("v1:cart:checkout"), {}, format="json").json()["data"]


def confirm(client, order_id):
    return client.post(reverse("v1:orders:confirm", kwargs={"order_id": order_id}))


def enable_high_value(settings, *, threshold="100", score=60, review=50, reject=100):
    settings.FRAUD = {
        **settings.FRAUD,
