...
def make_store(make_user):
    def _make(name: str = "Acme Store", country: str = ""):
        store = StoreService().create_store(owner=(owner := make_user()), data={"name": name})
        if country:
            store.country = country
            store.save(update_fields=["country"])
        return store, owner
...
def make_variant(db):
    ...
    def _make(store, *, price="100.00", stock=50):
        counter["n"] += 1
