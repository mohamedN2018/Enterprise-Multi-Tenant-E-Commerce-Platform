"""Shared fixtures for returns tests."""
...
def make_variant(db):
    ...
    def _make(store, *, price="50.00", stock=10):
        counter["n"] += 1
...
@pytest.fixture
def confirmed_order(store_client):
    """Buy + checkout + confirm; returns (buyer_client, buyer, order_dict)."""

    def _make(store, buyer, variant, qty=2):
